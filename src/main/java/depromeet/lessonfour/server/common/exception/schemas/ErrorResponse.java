...
public record ErrorResponse(int status, String message, List<ValidationError> errors) {
  ...
  public static ErrorResponse of(BaseErrorCode errorCode) {
    return new ErrorResponse(errorCode.getHttpStatus().value(), errorCode.getMessage(), null);
  }
  ...
  public static ErrorResponse of(BaseErrorCode errorCode, Set<ConstraintViolation<?>> violations) {
    return new ErrorResponse(
        errorCode.getHttpStatus().value(), errorCode.getMessage(), ValidationError.of(violations));
  }
  ...
  @JsonInclude(JsonInclude.Include.NON_NULL)
  public record ValidationError(String path, String field, String message) {
      ...
      return violations == null
          ...
          : violations.stream()
              .map(
                  violation ->
                      new ValidationError(
                          violation.getPropertyPath().toString(), null, violation.getMessage()))
              .toList();
    }

...
public class OAuthTokenClient {
  ...
  public OAuth2AccessTokenResponse requestToken(String code, SocialProvider socialProvider) {
    ClientRegistration registration =
        clientRegistrationRepository.findByRegistrationId(socialProvider.getValue());
    if (registration == null) {
      throw new ServerException(AuthErrorCode.OAUTH_PROVIDER_NOT_FOUND);
    }

    String redirectUri = redirectUriResolver.getRedirectUri(socialProvider);
    ...
    OAuth2AuthorizationResponse authResponse =
        OAuth2AuthorizationResponse.success(code).redirectUri(redirectUri).build();

    return getOAuth2AccessTokenResponse(authRequest, authResponse, registration);
  }

  private static OAuth2AccessTokenResponse getOAuth2AccessTokenResponse(
      OAuth2AuthorizationRequest authRequest,
      OAuth2AuthorizationResponse authResponse,
      ClientRegistration registration) {
